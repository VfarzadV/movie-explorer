...
import type { MovieType, TMDBMovie, TMDBShow } from "../types";
...
export default function GenreDetailsPage() {
    ...
    const renderPaginationItem = (current: number, type: string, element: React.ReactNode) => {
        if (type === 'page') {
            return (
                <button className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-md text-xs sm:text-sm font-medium transition-colors cursor-pointer ${currentPage === current
        ...
        if (type === 'prev') {
            return (
                <button disabled={currentPage === 1} className="px-2 sm:px-3 h-8 sm:h-10 flex items-center justify-center bg-[#111111] border border-gray-800 rounded-md text-xs sm:text-sm text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer">
                    Prev
                </button>
            );
        }
        if (type === 'next') {
            return (
                <button disabled={currentPage === totalPages} className="px-2 sm:px-3 h-8 sm:h-10 flex items-center justify-center bg-[#111111] border border-gray-800 rounded-md text-xs sm:text-sm text-gray-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer">
                    Next
                </button>
            );
        }
        if (type === 'jump-prev' || type === 'jump-next') {
            return <span className="text-gray-600 flex items-center justify-center h-8 sm:h-10 px-1 sm:px-2 tracking-widest text-sm">...</span>;
        }
        return element;
    };
    ...
    return (
        <div className="font-martel bg-linear-to-b from-[#191919] to-[#0A0A0A] min-h-screen flex flex-col">
            <Navbar />
            <div className="w-[95%] xl:w-[85%] mx-auto flex flex-col mt-10 md:mt-20 flex-1">
                ...
                {isLoading ? (
                    <div className="text-center text-red-500 py-10 md:py-20 text-lg md:text-xl animate-pulse">
